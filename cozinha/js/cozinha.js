...
(function(){
  ...
  async function start(){if(started)return;try{staff=await (window.NONNA_STAFF_AUTH?.restore?.()||null)}catch(_){staff=null}if(!staff){const gate=$('staffGate'),form=$('staffLogin'),err=$('staffError');gate.style.display='grid';form.dataset.handlerBound='1';form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),btn=form.querySelector('button');err.textContent='';btn.disabled=true;try{staff=await NONNA_STAFF_AUTH.signIn(String(f.get('email')).trim(),String(f.get('password')));if(window.DBRemote&&typeof DBRemote.getPedidosAsync==='function')DB._cachePedidos=await DBRemote.getPedidosAsync();gate.style.display='none';start()}catch(ex){err.textContent=ex.message||'Não foi possível entrar.'}finally{btn.disabled=false}};return}started=true;$('staffGate').style.display='none';$('connectionLabel').textContent='Conectado';render();DB.onChange(()=>render());tick=setInterval(clock,1000);$('soundToggle').onclick=()=>{sound=!sound;$('soundToggle').textContent=sound?'🔊':'🔇';$('soundToggle').setAttribute('aria-pressed',String(sound));if(sound)beep()};document.querySelector('.board').addEventListener('click',e=>{const b=e.target.closest('[data-status]');if(!b)return;const c=b.closest('.order-card');if(c)change(c.dataset.id,b.dataset.status,b)});}
