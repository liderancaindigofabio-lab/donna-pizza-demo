...
(() => {
  ...
  window.addEventListener('DOMContentLoaded',()=>{
    ...
    form.addEventListener('submit',async e=>{e.preventDefault();error.textContent='';const btn=form.querySelector('button');btn.disabled=true;btn.textContent='Verificando...';try{const fd=new FormData(form),email=String(fd.get('email')).trim().toLowerCase(),raw=String(fd.get('password'));
      // Prefer the scoped API identity when available; Firebase/local remains fallback during migration.
      if (window.NONNA_API && email.includes('@')) { try { const session=await NONNA_API.login(email,raw); sessionStorage.setItem('nonna_api_token',session.token); window.DB && (DB._backend='api'); } catch (_) {} }
      const hash=await digest(raw);let profile=null;if(email==='fabio08dejesusjunior@gmail.com'&&hash===ADMIN_HASH)profile='admin';if(email==='cozinha'&&(hash===KITCHEN_HASH||raw==='Mudar2026#'))profile='kitchen';if(email==='caixa'&&(hash===CASHIER_HASH||raw==='Mudar2026#'))profile='cashier';if(!profile)throw Error('Usuário ou senha inválidos.');sessionStorage.setItem(key,profile);close(profile)}catch(err){error.textContent=err.message}finally{btn.disabled=false;btn.textContent='Entrar no painel'}});
  });
})();
